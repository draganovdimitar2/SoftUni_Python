...
class AuctionHouseManagerApp:
    VALID_TYPES_ARTIFACTS: List[Type[BaseArtifact]] = [RenaissanceArtifact, ContemporaryArtifact]
    VALID_TYPES_COLLECTORS: List[Type[BaseCollector]] = [Museum, PrivateCollector]
    ...
    def register_artifact(self, artifact_type: str, artifact_name: str, artifact_price: float,
        ...
        artifact_class = next((a for a in self.VALID_TYPES_ARTIFACTS if a.__name__ == artifact_type), None)
    ...
    def register_collector(self, collector_type: str, collector_name: str) -> str:
        collector_class = next((c for c in self.VALID_TYPES_COLLECTORS if c.__name__ == collector_type), None)
